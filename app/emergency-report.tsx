  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F8FAF9] dark:bg-gray-900">
      <View className="flex-row items-center px-6 mt-4 mb-5">
        ...
        <Text className="ml-4 text-2xl font-extrabold text-[#111827] dark:text-white">Report Concern</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 110 }}>
        ...
        <Text className="mb-3 font-extrabold text-[#111827] dark:text-white">Type of concern</Text>
        <View className="mb-5 flex-row flex-wrap gap-2">
          {types.map(t => (
            <TouchableOpacity key={t.key}
              className={`flex-row items-center rounded-2xl px-4 py-3 ${selected === t.key ? "bg-red-500" : "border border-[#DCE8E1] bg-white dark:bg-gray-800"}`}
              onPress={() => setSelected(t.key)}>
              <Ionicons name={t.icon as any} size={16} color={selected === t.key ? "#fff" : "#6B7280"} />
              <Text className={`ml-2 font-extrabold ${selected === t.key ? "text-white" : "text-[#111827] dark:text-white"}`}>{t.label}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <Text className="mb-2 font-extrabold text-[#111827] dark:text-white">Animal type (optional)</Text>
        <TextInput value={animalType} onChangeText={setAnimalType}
          placeholder="e.g. Dog, Cat, Bird" placeholderTextColor="#9CA3AF"
        ...
        <Text className="mb-2 font-extrabold text-[#111827] dark:text-white">Location *</Text>
        <TextInput value={location} onChangeText={setLocation}
          placeholder="Street address or landmark" placeholderTextColor="#9CA3AF"
          className="mb-5 rounded-2xl border border-[#DCE8E1] bg-white px-4 py-4 text-[#111827] dark:bg-gray-800 dark:border-gray-700 dark:text-white" />

        <Text className="mb-2 font-extrabold text-[#111827] dark:text-white">Description *</Text>
        <TextInput value={description} onChangeText={setDescription} multiline
          placeholder="Describe what happened and what help is needed..." placeholderTextColor="#9CA3AF"
          className="mb-5 min-h-[140px] rounded-2xl border border-[#DCE8E1] bg-white px-4 py-4 text-[#111827] dark:bg-gray-800 dark:border-gray-700 dark:text-white"
          textAlignVertical="top" />

        <Text className="mb-2 font-extrabold text-[#111827] dark:text-white">Contact number (optional)</Text>
        <TextInput value={contactPhone} onChangeText={setContactPhone} keyboardType="phone-pad"
          ...
          className="mb-6 rounded-2xl border border-[#DCE8E1] bg-white px-4 py-4 text-[#111827] dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
