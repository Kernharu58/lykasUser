...
export default function Settings() {
  ...
  const [contactInfo, setContactInfo]         = useState({
    address: "CarePaws Shelter, Pampanga",
  ...
  useEffect(() => {
    const boot = async () => {
      ...
      if (meResult.status === "fulfilled") {
        const userData = meResult.value.data?.user ?? meResult.value.data;
        ...
        console.error("[Settings] /auth/me failed:", meResult.reason);
        setBootError(true);
      }

      if (settingsResult.status === "fulfilled" && settingsResult.value.data) {
        setContactInfo(prev => ({ ...prev, ...settingsResult.value.data }));
      } else if (settingsResult.status === "rejected") {
        console.error("[Settings] /settings failed:", settingsResult.reason);
      }
    };
    boot();
  }, []);
  ...
  const handleToggleNotifications = async (value: boolean) => {
    ...
    try {
      ...
      console.error("[Settings] Failed to sync notification pref:", e);
    }
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-bgSoft dark:bg-gray-900">
      ...
      <View className="flex-row items-center px-6 mt-4 mb-6">
        <TouchableOpacity
          onPress={() => router.back()}
          className="h-10 w-10 items-center justify-center rounded-full bg-white border border-border dark:bg-gray-800 dark:border-gray-700"
        >
      ...
      >
        ...
        {user && !user.emailVerified && (
          <View className="mb-6 rounded-3xl border border-amber-200 bg-amber-50 p-4 flex-row items-start">
            <View className="mr-3 h-9 w-9 rounded-full bg-amber-500 items-center justify-center">
            ...
            <View className="flex-1">
              ...
              <TouchableOpacity className="mt-2" onPress={() => router.push("/verify-email")}>
                <Text className="font-extrabold text-amber-700">Verify Email</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
        ...
        {bootError && (
          <View className="mb-6 flex-row items-center rounded-2xl border border-amber-200 bg-amber-50 p-4">
            ...
            <Text className="ml-3 flex-1 text-sm font-bold text-amber-800">
              Could not load profile data. Check your connection and restart the screen.
            </Text>
          </View>
        )}
        ...
        <View className="items-center mb-8">
          <View className="relative">
            <TouchableOpacity
              className="h-28 w-28 rounded-full bg-primary items-center justify-center overflow-hidden border-4 border-white shadow-sm"
              onPress={pickImage}
            ...
            >
              {uploadingImage ? (
                ...
                <Image source={{ uri: profilePicture }} className="h-full w-full" resizeMode="cover" />
              ) : (
                <Text className="text-4xl font-extrabold text-white">{initial}</Text>
              )}
            </TouchableOpacity>
            <TouchableOpacity
              className="absolute bottom-1 right-0 h-10 w-10 rounded-full bg-white border border-border items-center justify-center shadow-sm"
              onPress={pickImage}
            >
          ...
          <View className="mt-4 flex-row items-center">
            <Text className="text-3xl font-extrabold text-gray-900 dark:text-white">
              {displayName || "CarePaws User"}
            </Text>
            <TouchableOpacity
              className="ml-3 h-9 w-9 rounded-full bg-mintBg items-center justify-center"
              onPress={() => { setEditName(displayName); setEditModalVisible(true); }}
            >
        ...
        <View className="rounded-3xl bg-white p-5 shadow-sm border border-border dark:bg-gray-800 dark:border-gray-700 mb-6">
          <View className="flex-row justify-between items-center mb-5 pb-5 border-b border-gray100">
            <View className="flex-row items-center">
              <View className="mr-4 h-11 w-11 rounded-full bg-mintBg items-center justify-center">
                <Ionicons name="notifications-outline" size={21} color={GREEN} />
              </View>
              <Text className="text-base font-extrabold text-gray-900 dark:text-white">Push Notifications</Text>
            </View>
            <Switch
              value={notificationsEnabled}
              onValueChange={handleToggleNotifications}
              trackColor={{ false: COLORS.gray200, true: COLORS.mintLight }}
              thumbColor={notificationsEnabled ? GREEN : COLORS.gray50}
            />
          </View>
          <View className="flex-row justify-between items-center">
            <View className="flex-row items-center">
              <View className="mr-4 h-11 w-11 rounded-full bg-mintBg items-center justify-center">
                <Ionicons name={isDarkMode ? "moon" : "moon-outline"} size={21} color={GREEN} />
              </View>
              <Text className="text-base font-extrabold text-gray-900 dark:text-white">Dark Mode</Text>
            </View>
            <Switch
              value={isDarkMode}
              onValueChange={toggleDarkMode}
              trackColor={{ false: COLORS.gray200, true: COLORS.mintLight }}
              thumbColor={isDarkMode ? GREEN : COLORS.gray50}
            />
          </View>
        </View>
        ...
        {/* Log Out */}
        <TouchableOpacity
          className="mb-6 rounded-2xl border border-red-100 bg-red-50 py-4 flex-row items-center justify-center"
      ...
      <Modal visible={isEditModalVisible} transparent animationType="fade">
        <View className="flex-1 bg-black/50 justify-center items-center px-4">
          <View className="bg-white dark:bg-gray-800 rounded-3xl p-6 w-full max-w-sm shadow-xl">
            ...
            <TextInput
              className="bg-bgSoft border border-border rounded-2xl px-4 py-3 text-gray-900 mb-6"
              value={editName}
